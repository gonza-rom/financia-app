// features/empresas/editar-empresa-dialog.tsx
"use client";
...
import type { FormularioEmpresa, EmpresaResumen } from "@/types/empresas";
...
export function EditarEmpresaDialog({ empresa, open, onOpenChange }: EditarEmpresaDialogProps) {
  const [isPending, startTransition] = useTransition();
  const { toast } = useToast();

  const { register, handleSubmit, watch, setValue } = useForm<FormularioEmpresa>({
    defaultValues: {
  ...
  function onSubmit(data: FormularioEmpresa) {
    startTransition(async () => {
      const result = await actualizarEmpresaAction(empresa.id, {
        ...data,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 mt-2">
          ...
          <div className="space-y-2">
            <Label>Moneda</Label>
            <Select defaultValue={empresa.moneda} onValueChange={(v) => setValue("moneda", v)}>
